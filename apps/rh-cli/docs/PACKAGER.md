...
| `*.md` matching a resource | Embedded as `resource.text` |
| Other `*.md` files | `package/other/*.md` |
...
Create `bp-profiles/StructureDefinition-bp-observation.md`:
...
During the build, this markdown is converted to XHTML and embedded as `resource.text` in the
compiled `StructureDefinition-bp-observation.json`.
