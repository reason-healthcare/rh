...
fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Batch { input_dir, output_dir, config, pattern } => {
...
fn init_config(config_path: &Path) -> Result<()> {
    info!("Initializing codegen configuration at: {}", config_path.display());
...
/// Generate Rust types from a single FHIR StructureDefinition
fn generate_single(input_path: &Path, output_path: Option<&Path>, config_path: &Path) -> Result<()> {
    ...
    let structure_def = generator.load_structure_definition(input_path)
        .map_err(|e| anyhow::anyhow!("Failed to load StructureDefinition: {}", e))?;
    
    info!("Loaded StructureDefinition: {} ({})", structure_def.name, structure_def.id);
    ...
    // Generate the code
    generator.generate_to_file(&structure_def, &output_file_path)
...
/// Generate Rust types from multiple FHIR StructureDefinitions
fn generate_batch(input_dir: &Path, output_dir: &Path, config_path: &Path, pattern: &str) -> Result<()> {
    info!("Loading configuration from: {}", config_path.display());
    
    let config: CodegenConfig = utils::load_config_from_file(&config_path.to_string_lossy())
        .map_err(|e| anyhow::anyhow!("Failed to load config: {}", e))?;
    
    info!("Processing FHIR StructureDefinitions from: {}", input_dir.display());
...
fn process_single_file(
    ...
    // Generate the code
    generator.generate_to_file(&structure_def, &output_path)
        .map_err(|e| anyhow::anyhow!("Failed to generate code: {}", e))?;
