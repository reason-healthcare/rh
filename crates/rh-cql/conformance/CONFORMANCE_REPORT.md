...
|---------|--------|-------------|
| `^` power operator | ✅ Implemented | Parses as `BinaryOperator::Power` with correct precedence |
| `predecessor of` | ✅ Implemented | Parses as `UnaryOperator::Predecessor` |
| `successor of` | ✅ Implemented | Parses as `UnaryOperator::Successor` |
| `minimum Type` | ✅ Implemented | Parses as `Expression::MinValue(TypeSpecifier)` |
| `maximum Type` | ✅ Implemented | Parses as `Expression::MaxValue(TypeSpecifier)` |
| `aggregate` clause | ✅ Implemented | Query aggregate clause with optional `distinct` and `starting` |
| Multiple `let` items | ✅ Implemented | `let a: x, b: y` comma-separated items in one clause |
