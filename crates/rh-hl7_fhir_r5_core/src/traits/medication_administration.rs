...
pub trait MedicationAdministrationMutators: DomainResourceMutators {
    ...
    /// # Example
    /// ```rust
    /// use rh_hl7_fhir_r5_core::resources::medication_administration::MedicationAdministration;
    /// use rh_hl7_fhir_r5_core::traits::medication_administration::MedicationAdministrationMutators;
    ///
...
pub trait MedicationAdministrationExistence: DomainResourceExistence {
    ...
    /// Returns true if the occurence field is present (Some).
    fn has_occurence(&self) -> bool;
    /// Returns true if the recorded field is present (Some).
    fn has_recorded(&self) -> bool;
    /// Returns true if the is_sub_potent field is present (Some).
    fn has_is_sub_potent(&self) -> bool;
    /// Returns true if the sub_potent_reason field is not empty.
