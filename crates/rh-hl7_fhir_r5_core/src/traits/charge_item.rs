...
pub trait ChargeItemAccessors: DomainResourceAccessors {
    ...
    /// Returns a reference to the unitPriceComponent field.
    fn unit_price_component(&self) -> Option<MonetaryComponent>;
    /// Returns a reference to the totalPriceComponent field.
...
pub trait ChargeItemMutators: DomainResourceMutators {
    ...
    /// # Example
    /// ```rust
    /// use rh_hl7_fhir_r5_core::resources::charge_item::ChargeItem;
    /// use rh_hl7_fhir_r5_core::traits::charge_item::ChargeItemMutators;
    ///
    ...
    /// Sets the unitPriceComponent field and returns self for chaining.
    ...
    /// Adds an item to the product field and returns self for chaining.
...
pub trait ChargeItemExistence: DomainResourceExistence {
    ...
    /// Returns true if the unit_price_component field is present (Some).
    fn has_unit_price_component(&self) -> bool;
    /// Returns true if the total_price_component field is present (Some).
    ...
    /// Returns true if the product field is not empty.
