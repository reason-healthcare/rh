...
use crate::bindings::questionnaire_answer_constraint::QuestionnaireAnswerConstraint;
use crate::bindings::questionnaire_disabled_display::QuestionnaireDisabledDisplay;
...
pub struct QuestionnaireItem {
    ...
    pub disabled_display: Option<QuestionnaireDisabledDisplay>,
    /// Extension element for the 'disabledDisplay' primitive field. Contains metadata and extensions.
    #[serde(rename = "_disabledDisplay")]
    pub _disabled_display: Option<Element>,
    ...
    pub answer_constraint: Option<QuestionnaireAnswerConstraint>,
    /// Extension element for the 'answerConstraint' primitive field. Contains metadata and extensions.
    #[serde(rename = "_answerConstraint")]
    pub _answer_constraint: Option<Element>,
...
impl Default for QuestionnaireItem {
    fn default() -> Self {
        Self {
            ...
            disabled_display: Default::default(),
            _disabled_display: Default::default(),
            ...
            answer_constraint: Default::default(),
            _answer_constraint: Default::default(),
...
pub static INVARIANTS: once_cell::sync::Lazy<Vec<rh_foundation::Invariant>> =
    ...
    rh_foundation::Invariant::new("que-10", rh_foundation::Severity::Error, "Maximum length can only be declared for simple question types", "(type in ('boolean' | 'decimal' | 'integer' | 'string' | 'text' | 'url')) or answerConstraint='optionOrString' or maxLength.empty()"),
    rh_foundation::Invariant::new("que-11", rh_foundation::Severity::Error, "If one or more answerOption is present, initial cannot be present.  Use answerOption.initialSelected instead", "answerOption.empty() or initial.empty()"),
    rh_foundation::Invariant::new("que-12", rh_foundation::Severity::Error, "If there are more than one enableWhen, enableBehavior must be specified", "enableWhen.count() > 1 implies enableBehavior.exists()"),
    rh_foundation::Invariant::new("que-13", rh_foundation::Severity::Error, "Can only have multiple initial values for repeating items", "repeats=true or initial.count() <= 1"),
    rh_foundation::Invariant::new("que-14", rh_foundation::Severity::Warning, "Can only have answerConstraint if answerOption or answerValueSet are present.  (This is a warning because extensions may serve the same purpose)", "answerConstraint.exists() implies answerOption.exists() or answerValueSet.exists()"),
    rh_foundation::Invariant::new("que-15", rh_foundation::Severity::Warning, "Link ids should be 255 characters or less", "$this.length() <= 255"),
    rh_foundation::Invariant::new("que-1a", rh_foundation::Severity::Error, "Group items must have nested items when Questionanire is complete", "(type='group' and %resource.status='complete') implies item.empty().not()"),
    rh_foundation::Invariant::new("que-1b", rh_foundation::Severity::Warning, "Groups should have items", "type='group' implies item.empty().not()"),
    rh_foundation::Invariant::new("que-1c", rh_foundation::Severity::Error, "Display items cannot have child items", "type='display' implies item.empty()"),
    rh_foundation::Invariant::new("que-2", rh_foundation::Severity::Error, "The link ids for groups and questions must be unique within the questionnaire", "descendants().linkId.isDistinct()"),
    rh_foundation::Invariant::new("que-3", rh_foundation::Severity::Error, "Display items cannot have a \"code\" asserted", "type!='display' or code.empty()"),
    rh_foundation::Invariant::new("que-4", rh_foundation::Severity::Error, "A question cannot have both answerOption and answerValueSet", "answerOption.empty() or answerValueSet.empty()"),
    ...
    rh_foundation::Invariant::new("que-6", rh_foundation::Severity::Error, "Required and repeat aren't permitted for display items", "type!='display' or (required.empty() and repeats.empty())"),
    rh_foundation::Invariant::new("que-7", rh_foundation::Severity::Error, "If the operator is 'exists', the value must be a boolean", "operator = 'exists' implies (answer is boolean)"),
    rh_foundation::Invariant::new("que-8", rh_foundation::Severity::Error, "Initial values can't be specified for groups or display items", "(type!='group' and type!='display') or initial.empty()"),
    rh_foundation::Invariant::new("que-9", rh_foundation::Severity::Error, "Read-only can't be specified for \"display\" items", "type!='display' or readOnly.empty()"),
]
    });
...
pub static BINDINGS: once_cell::sync::Lazy<Vec<rh_foundation::ElementBinding>> =
    once_cell::sync::Lazy::new(|| {
        vec![
    rh_foundation::ElementBinding::new("Questionnaire.item.answerConstraint", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/questionnaire-answer-constraint|5.0.0").with_description("Indicates differnt modes for restricting values when options or valueset are specified"),
    rh_foundation::ElementBinding::new("Questionnaire.item.disabledDisplay", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/questionnaire-disabled-display|5.0.0").with_description("Defines how disabled elements should be rendered"),
    rh_foundation::ElementBinding::new("Questionnaire.item.enableBehavior", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/questionnaire-enable-behavior|5.0.0").with_description("Controls how multiple enableWhen values are interpreted -  whether all or any must be true."),
    rh_foundation::ElementBinding::new("Questionnaire.item.enableWhen.operator", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/questionnaire-enable-operator|5.0.0").with_description("The criteria by which a question is enabled."),
    rh_foundation::ElementBinding::new("Questionnaire.item.type", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/item-type|5.0.0").with_description("Distinguishes groups from questions and display text and indicates data type for questions."),
    rh_foundation::ElementBinding::new("Questionnaire.language", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/all-languages|5.0.0").with_description("IETF language tag for a human language"),
    rh_foundation::ElementBinding::new("Questionnaire.status", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/publication-status|5.0.0").with_description("The lifecycle status of an artifact."),
    rh_foundation::ElementBinding::new("Questionnaire.subjectType", rh_foundation::BindingStrength::Required, "http://hl7.org/fhir/ValueSet/resource-types|5.0.0").with_description("One of the resource types defined as part of this version of FHIR."),
]
    });
...
pub static CARDINALITIES: once_cell::sync::Lazy<Vec<rh_foundation::ElementCardinality>> =
    once_cell::sync::Lazy::new(|| {
        vec![
            ...
            rh_foundation::ElementCardinality::new("Questionnaire.versionAlgorithm[x]", 0, Some(1)),
            ...
            rh_foundation::ElementCardinality::new("Questionnaire.copyrightLabel", 0, Some(1)),
