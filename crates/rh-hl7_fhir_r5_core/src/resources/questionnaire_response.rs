...
impl Default for QuestionnaireResponse {
    fn default() -> Self {
        Self {
            ...
            questionnaire: StringType::default(),
...
pub static INVARIANTS: once_cell::sync::Lazy<Vec<rh_foundation::Invariant>> =
    ...
    rh_foundation::Invariant::new("qrs-1", rh_foundation::Severity::Error, "Item cannot contain both item and answer", "(answer.exists() and item.exists()).not()"),
    rh_foundation::Invariant::new("qrs-2", rh_foundation::Severity::Error, "Repeated answers are combined in the answers array of a single item", "repeat(answer|item).select(item.where(answer.value.exists()).linkId.isDistinct()).allTrue()"),
]
    });
...
pub static CARDINALITIES: once_cell::sync::Lazy<Vec<rh_foundation::ElementCardinality>> =
    once_cell::sync::Lazy::new(|| {
        vec![
            ...
            rh_foundation::ElementCardinality::new("QuestionnaireResponse.identifier", 0, None),
...
impl crate::validation::ValidatableResource for QuestionnaireResponse {
