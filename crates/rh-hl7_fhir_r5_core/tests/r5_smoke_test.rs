...
use rh_hl7_fhir_r5_core::{
    ...
    resources::{
        patient::Patient, subscription_status::SubscriptionStatus, transport::Transport,
    },
    traits::patient::{PatientExistence, PatientMutators},
    traits::resource::{ResourceExistence, ResourceMutators},
    traits::domain_resource::DomainResourceMutators,
    traits::subscription_status::{SubscriptionStatusAccessors, SubscriptionStatusMutators},
...
fn test_patient_gender_binding_enum() {
    let patient = <Patient as PatientMutators>::new()
        .set_gender(AdministrativeGender::Female);
    let json = serde_json::to_string(&patient.gender).expect("serialize");
    assert!(json.contains("female"), "gender should serialize to 'female', got {json}");
}
...
fn test_patient_identifier() {
    ...
    assert_eq!(
        patient.identifier.as_ref().and_then(|v| v[0].value.as_deref()),
        Some("MRN-123")
    );
}
...
fn test_patient_serde_roundtrip() {
    ...
    assert!(json.contains("\"Jones\""), "serialized JSON should contain family name");
...
fn test_transport_serde_roundtrip() {
    ...
    transport.status = Some(TransportStatus::InProgress);
    transport.intent = TransportIntent::Order;
    transport.base.base.id = Some("t-001".to_string());

    let json = serde_json::to_string(&transport).expect("serialize");
    assert!(json.contains("in-progress"), "status should serialize to 'in-progress'");
...
fn test_subscription_status_integer64_field() {
    let mut ss = SubscriptionStatus::default();
    ss.events_since_subscription_start = Some("9007199254740992".to_string());
...
fn test_subscription_status_status_is_string_type() {
    ...
    let mut ss = SubscriptionStatus::default();
    ss.status = Some("active".to_string()); // compile-time proof it's Option<String>
    ss.type_ = SubscriptionNotificationType::EventNotification;
...
fn test_subscription_status_serde_roundtrip() {
    let mut ss = SubscriptionStatus::default();
    ss.status = Some("active".to_string());
    ss.type_ = SubscriptionNotificationType::Heartbeat;
    ss.events_since_subscription_start = Some("100".to_string());

    let json = serde_json::to_string(&ss).expect("serialize");
    assert!(json.contains("\"active\""), "status should appear in JSON");
    assert!(json.contains("heartbeat"), "type should serialize to 'heartbeat'");
...
fn test_biologically_derived_product_dispense_construction() {
    ...
    let mut dispense = BiologicallyDerivedProductDispense::default();
    dispense.status = BiologicallyderivedproductdispenseStatus::InProgress;
    dispense.product = Reference {
        reference: Some("BiologicallyDerivedProduct/bdp-001".to_string()),
        ..Default::default()
    };
    dispense.patient = Reference {
        reference: Some("Patient/p-001".to_string()),
        ..Default::default()
    };

    let status_json =
        serde_json::to_string(&dispense.status).expect("serialize status");
    assert!(status_json.contains("in-progress"));
    assert_eq!(
    ...
    assert_eq!(
        dispense.patient.reference.as_deref(),
        Some("Patient/p-001")
    );
}
...
fn test_patient_has_cardinalities() {
    ...
    assert!(id_card.is_some(), "Patient.id cardinality should be defined");
