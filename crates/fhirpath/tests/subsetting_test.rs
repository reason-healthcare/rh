...
mod subsetting_tests {
    ...
    fn evaluate_expression(data: &Value, expression: &str) -> Result<FhirPathValue, Box<dyn std::error::Error>> {
    ...
    fn test_single_function_error_multiple_items() {
        ...
        assert!(result.unwrap_err().to_string().contains("single() requires exactly one item"));
    }
    ...
    fn test_single_function_error_empty_collection() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("single() cannot be called on empty collection"));
    }
