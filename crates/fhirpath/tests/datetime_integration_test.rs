...
use serde_json::json;
use chrono::{DateTime, NaiveDate, NaiveTime, Utc, Local, Timelike};
...
fn test_now_function() {
    ...
    match result {
        FhirPathValue::DateTime(dt_str) => {
            ...
            assert!(diff <= 1, "now() should return current time within 1 second");
        }
...
fn test_today_function() {
    ...
    match result {
        FhirPathValue::Date(date_str) => {
            ...
            assert_eq!(parsed_date, system_today, "today() should return current date");
        }
...
fn test_time_of_day_function() {
    ...
    match result {
        FhirPathValue::Time(time_str) => {
            ...
            assert!(diff <= 1, "timeOfDay() should return current time within 1 second, diff: {} seconds", diff);
        }
...
fn test_datetime_functions_no_parameters() {
    ...
    assert!(result.unwrap_err().to_string().contains("takes no parameters"));
...
fn test_datetime_functions_in_expressions() {
    ...
    let result = evaluator.evaluate(&expr, &context).unwrap();
    assert_eq!(result, FhirPathValue::Boolean(true), "now().exists() should be true");

    let expr = parser.parse("today().exists()").unwrap();
    let result = evaluator.evaluate(&expr, &context).unwrap();
    assert_eq!(result, FhirPathValue::Boolean(true), "today().exists() should be true");

    let expr = parser.parse("timeOfDay().exists()").unwrap();
    let result = evaluator.evaluate(&expr, &context).unwrap();
    assert_eq!(result, FhirPathValue::Boolean(true), "timeOfDay().exists() should be true");
}
...
fn test_multiple_datetime_function_calls() {
    ...
    assert_ne!(result1, result2, "Multiple now() calls should return different values");
    ...
    assert_eq!(today1, today2, "Multiple today() calls should return same date");
}
