...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    test_expression(&parser, &evaluator, &context, 
        "getResourceKey()", 
        "Get Patient resource key for SQL joins")?;
    ...
    test_expression(&parser, &evaluator, &obs_context,
    ...
    println!("Demonstrating that resource keys and reference keys are consistent:");
    
    // Get the patient resource key
    let patient_key_result = parser.parse("getResourceKey()").and_then(|parsed| {
        evaluator.evaluate(&parsed, &context)
    })?;
    
    // Get the reference key from the observation
    let reference_key_result = parser.parse("subject.getReferenceKey()").and_then(|parsed| {
        evaluator.evaluate(&parsed, &obs_context)
    })?;
    ...
    if patient_key_result == reference_key_result {
        println!("✅ Keys match! This enables efficient SQL joins.");
    } else {
    ...
    test_expression(&parser, &evaluator, &bundle_context,
    ...
    // Find resources that reference the same patient
    test_expression(&parser, &evaluator, &bundle_context,
    ...
    println!("• Extract resource keys for SQL primary keys");
    println!("• Extract reference keys for SQL foreign keys");  
                ...
                Ok(result) => {
                    let display_result = match result {
                        ...
                        FhirPathValue::Collection(ref coll) => {
                            let items: Vec<String> = coll.iter().map(|item| match item {
                                FhirPathValue::String(s) => format!("\"{}\"", s),
                                FhirPathValue::Integer(i) => i.to_string(),
                                FhirPathValue::Boolean(b) => b.to_string(),
                                other => format!("{:?}", other),
