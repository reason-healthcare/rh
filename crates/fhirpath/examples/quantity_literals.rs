...
fn main() {
    ...
    // Weight measurements
    demo_quantity(&parser, &evaluator, "70.5'kg'", "Patient weight in kilograms");
    demo_quantity(&parser, &evaluator, "155'lb'", "Patient weight in pounds");
    
    // Temperature measurements  
    demo_quantity(&parser, &evaluator, "37.2'Cel'", "Body temperature in Celsius");
    demo_quantity(&parser, &evaluator, "98.9'[degF]'", "Body temperature in Fahrenheit");
    
    // Blood pressure
    demo_quantity(&parser, &evaluator, "120'mm[Hg]'", "Systolic blood pressure");
    demo_quantity(&parser, &evaluator, "80'mm[Hg]'", "Diastolic blood pressure");
    
    // Medication dosages
    demo_quantity(&parser, &evaluator, "5'mg'", "Medication dose in milligrams");
    ...
    demo_quantity(&parser, &evaluator, "30'd'", "Prescription duration in days");
    ...
    demo_calculation(&parser, &evaluator, "70'kg' + 2.5'kg'", "Weight gain calculation");
    demo_calculation(&parser, &evaluator, "120'mm[Hg]' - 15'mm[Hg]'", "Blood pressure change");
    
    // Multiplication and division
    demo_calculation(&parser, &evaluator, "5'mg' * 2", "Double the medication dose");
    demo_calculation(&parser, &evaluator, "100'mL' / 4", "Divide fluid into portions");
    demo_calculation(&parser, &evaluator, "10'mg' / 5'mg'", "Dose ratio (dimensionless)");
    
    // Complex calculations
    demo_calculation(&parser, &evaluator, "(75'kg' + 5'kg') * 0.2", "20% of adjusted weight");
    ...
    demo_error(&parser, &evaluator, "5'mg' + 3'kg'", "Adding incompatible units");
    demo_error(&parser, &evaluator, "10'L' - 2'mL'", "Subtracting different volume units");
    demo_error(&parser, &evaluator, "120'mm[Hg]' + 5", "Adding number to quantity with units");
    ...
    demo_quantity(&parser, &evaluator, "4.5'10*6/uL'", "White blood cell count");
...
fn demo_quantity(parser: &FhirPathParser, evaluator: &FhirPathEvaluator, expression: &str, description: &str) {
    let empty_context = EvaluationContext::new(json!({}));
    match parser.parse(expression) {
        Ok(expr) => {
            match evaluator.evaluate(&expr, &empty_context) {
                Ok(result) => {
                    match result {
                        ...
                        _ => {
                            println!("  {}: {} = {:?} (unexpected type)", description, expression, result);
                        }
                    }
                }
                Err(e) => {
                    println!("  {}: {} → ERROR: {}", description, expression, e);
                }
            }
        }
        Err(e) => {
            println!("  {}: {} → PARSE ERROR: {}", description, expression, e);
        }
    }
}

fn demo_calculation(parser: &FhirPathParser, evaluator: &FhirPathEvaluator, expression: &str, description: &str) {
    let empty_context = EvaluationContext::new(json!({}));
    match parser.parse(expression) {
        Ok(expr) => {
            match evaluator.evaluate(&expr, &empty_context) {
...
fn demo_error(parser: &FhirPathParser, evaluator: &FhirPathEvaluator, expression: &str, description: &str) {
    let empty_context = EvaluationContext::new(json!({}));
    match parser.parse(expression) {
        Ok(expr) => {
            match evaluator.evaluate(&expr, &empty_context) {
                Ok(result) => {
                    println!("  {}: {} = {:?} (unexpected success)", description, expression, result);
                }
                Err(e) => {
                    println!("  {}: {} → ✓ Correctly handled error: {}", description, expression, e);
                }
            }
        }
        Err(e) => {
