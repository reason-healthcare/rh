        ...
        FhirPathValue::Object(obj) => {
            ...
            if let Some(resource_type) = obj.get("resourceType") {
                if let Some(resource_type_str) = resource_type.as_str() {
                    // Extract the resource ID and combine with resource type
                    if let Some(id_value) = obj.get("id") {
                        if let Some(id_str) = id_value.as_str() {
                            let resource_key = format!("{resource_type_str}/{id_str}");
                            Ok(FhirPathValue::String(resource_key))
                        } else {
...
fn get_reference_key_function(
    ...
    if params.len() > 1 {
        return Err(FhirPathError::InvalidOperation {
            message: "getReferenceKey() takes at most one parameter (expected resource type)"
                .to_string(),
        });
    }
        ...
        match &params[0] {
            ...
            _ => {
                return Err(FhirPathError::TypeError {
                    message: "getReferenceKey() parameter must be a string resource type"
                        .to_string(),
                })
            }
        }
        ...
        FhirPathValue::Object(obj) => {
            ...
            if let Some(reference_value) = obj.get("reference") {
                if let Some(reference_str) = reference_value.as_str() {
                    extract_reference_key(reference_str, expected_type)
                } else {
...
///
/// # Returns
/// * `Ok(FhirPathValue::String)` - The full reference string if valid and type matches
...
fn extract_reference_key(
    ...
    if let Some(slash_pos) = reference_str.find('/') {
        ...
        if !resource_id.is_empty() {
            return Ok(FhirPathValue::String(reference_str.to_string()));
        }
    }
