...
pub fn register_variables(variables: &mut HashMap<String, VariableResolver>) {
    ...
    variables.insert(
        "context".to_string(),
        Box::new(|name: &str, context: &EvaluationContext| {
            resolve_context_variable(name, context)
        }),
    );
    ...
    variables.insert(
        "ucum".to_string(),
        Box::new(|name: &str, context: &EvaluationContext| {
            resolve_ucum_variable(name, context)
        }),
    );
    ...
    variables.insert(
        "sct".to_string(),
        Box::new(|name: &str, context: &EvaluationContext| {
            resolve_sct_variable(name, context)
        }),
    );
    ...
    variables.insert(
        "loinc".to_string(),
        Box::new(|name: &str, context: &EvaluationContext| {
            resolve_loinc_variable(name, context)
        }),
    );
}
...
fn resolve_ucum_variable(
    ...
    match name {
        "ucum" => {
            Ok(Some(FhirPathValue::String("http://unitsofmeasure.org".to_string())))
        }
        _ => Ok(None),
    }
}
...
fn resolve_sct_variable(
    ...
    match name {
        "sct" => {
            Ok(Some(FhirPathValue::String("http://snomed.info/sct".to_string())))
        }
        _ => Ok(None),
    }
}
...
fn resolve_loinc_variable(
    ...
    match name {
        "loinc" => {
            Ok(Some(FhirPathValue::String("http://loinc.org".to_string())))
        }
        _ => Ok(None),
    }
}
...
mod tests {
    ...
    fn test_resource_variable_resolution() {
        ...
        match result.unwrap() {
            FhirPathValue::Object(obj) => {
                assert_eq!(obj.get("resourceType").unwrap().as_str().unwrap(), "Patient");
    ...
    fn test_ucum_variable_resolution() {
        ...
        assert!(result.is_some());
        assert_eq!(result.unwrap(), FhirPathValue::String("http://unitsofmeasure.org".to_string()));
    }
    ...
    fn test_sct_variable_resolution() {
        ...
        assert!(result.is_some());
        assert_eq!(result.unwrap(), FhirPathValue::String("http://snomed.info/sct".to_string()));
    }
    ...
    fn test_loinc_variable_resolution() {
        ...
        assert!(result.is_some());
        assert_eq!(result.unwrap(), FhirPathValue::String("http://loinc.org".to_string()));
    }
    ...
    fn test_unknown_variable() {
        ...
        assert!(resolve_resource_variable("unknown", &context).unwrap().is_none());
        assert!(resolve_context_variable("unknown", &context).unwrap().is_none());
        assert!(resolve_root_resource_variable("unknown", &context).unwrap().is_none());
        assert!(resolve_ucum_variable("unknown", &context).unwrap().is_none());
        assert!(resolve_sct_variable("unknown", &context).unwrap().is_none());
        assert!(resolve_loinc_variable("unknown", &context).unwrap().is_none());
    }
}
