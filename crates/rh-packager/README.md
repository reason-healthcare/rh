...
Used by `rh package` subcommands in the `rh` CLI.
...
```
rh package build <dir>   # Build package → output/ + <name>-<version>.tgz
rh package lock  <dir>   # Resolve canonical refs → write fhir-lock.json
rh package check <dir>   # Validate source (no output written)
rh package pack  <dir>   # Pack expanded output/ → .tgz
```
...
> **Note:** `before_pack`/`after_pack` are only invoked by `rh package build` (which runs pack
> as a final step). Running `rh package pack` on an already-built directory skips hooks because
...
Generated by `rh package lock`. Records resolved canonical versions for reproducible builds.
