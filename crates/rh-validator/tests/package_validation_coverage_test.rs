...
#[test]
fn questionnaire_item_answer_option_invariants_are_reported() {
    let validator = FhirValidator::new(FhirVersion::R4, None).unwrap();
    ...
    let questionnaire = json!({
        ...
        "item": [
            ...
            },
            {
                "linkId": "q3",
                "type": "boolean",
                "answerValueSet": "http://example.org/fhir/ValueSet/boolean-options"
            }
        ]
    });
    ...
    let que5_errors: Vec<_> = result
        .issues
        .iter()
        .filter(|issue| issue.severity == Severity::Error && issue.message.contains("que-5"))
        .collect();
    assert_eq!(
        que5_errors.len(),
        2,
        "expected que-5 errors for boolean answerOption and answerValueSet, got: {:?}",
        result.issues
    );
    assert!(que5_errors.iter().any(|issue| {
        issue.path.as_deref() == Some("Questionnaire.item[0]")
            && issue.message.contains("boolean item has answerOption")
    }));
    assert!(que5_errors.iter().any(|issue| {
        issue.path.as_deref() == Some("Questionnaire.item[2]")
            && issue.message.contains("boolean item has answerValueSet")
    }));
    ...
    assert!(
        result.issues.iter().any(|issue| {
            issue.severity == Severity::Warning
                && issue
                    .message
                    .contains("Questionnaire answerOption Coding has code but no system")
                && issue.path.as_deref()
                    == Some("Questionnaire.item[0].answerOption[0].valueCoding")
        }),
        "expected warning for answerOption Coding code without system, got: {:?}",
        result.issues
    );
}
