...
pub static INVARIANTS: once_cell::sync::Lazy<Vec<rh_foundation::Invariant>> =
    ...
    rh_foundation::Invariant::new("qrs-1", rh_foundation::Severity::Error, "Nested item can't be beneath both item and answer", "(answer.exists() and item.exists()).not()").with_xpath("not(exists(f:answer) and exists(f:item))"),
]
    });
