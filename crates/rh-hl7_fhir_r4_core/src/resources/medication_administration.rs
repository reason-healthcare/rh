...
pub static INVARIANTS: once_cell::sync::Lazy<Vec<rh_foundation::Invariant>> =
    ...
    rh_foundation::Invariant::new("mad-1", rh_foundation::Severity::Error, "SHALL have at least one of dosage.dose or dosage.rate[x]", "dose.exists() or rate.exists()").with_xpath("exists(f:dose) or exists(f:*[starts-with(local-name(.), 'rate')])"),
]
    });
