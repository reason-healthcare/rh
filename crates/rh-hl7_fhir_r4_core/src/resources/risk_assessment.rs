...
pub static INVARIANTS: once_cell::sync::Lazy<Vec<rh_foundation::Invariant>> =
    ...
    rh_foundation::Invariant::new("ras-1", rh_foundation::Severity::Error, "low and high must be percentages, if present", "(low.empty() or ((low.code = '%') and (low.system = %ucum))) and (high.empty() or ((high.code = '%') and (high.system = %ucum)))").with_xpath("(not(f:low) or f:low[f:code/@value='%' and f:system/@value='http://unitsofmeasure.org']) and (not(f:high) or f:high[f:code/@value='%' and f:system/@value='http://unitsofmeasure.org'])"),
    rh_foundation::Invariant::new("ras-2", rh_foundation::Severity::Error, "Must be <= 100", "probability is decimal implies (probability as decimal) <= 100").with_xpath("not(f:probabilityDecimal) or f:probabilityDecimal/@value <= 100"),
]
    });
