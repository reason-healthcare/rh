...
pub static INVARIANTS: once_cell::sync::Lazy<Vec<rh_foundation::Invariant>> =
    once_cell::sync::Lazy::new(|| {
        vec![
    rh_foundation::Invariant::new("ctm-1", rh_foundation::Severity::Error, "CareTeam.participant.onBehalfOf can only be populated when CareTeam.participant.member is a Practitioner", "onBehalfOf.exists() implies (member.resolve().iif(empty(), true, ofType(Practitioner).exists()))").with_xpath("starts-with(f:member/f:reference/@value, 'Practitioner/') or contains(f:member/f:reference/@value, '/Practitioner/') or exists(ancestor::*/f:contains/f:Practitioner/f:id[@value=substring-after(current()/f:member/f:reference/@value, '#')]) or not(exists(f:onBehalfOf))"),
...
impl crate::traits::care_team::CareTeamAccessors for CareTeam {
    ...
    fn managing_organization(&self) -> &[Reference] {
        self.managing_organization.as_slice()
    }
...
impl crate::traits::care_team::CareTeamMutators for CareTeam {
    ...
    fn set_managing_organization(self, value: Vec<Reference>) -> Self {
        let mut resource = self.clone();
        resource.managing_organization = value;
        resource
    }
    fn add_managing_organization(self, item: Reference) -> Self {
        let mut resource = self.clone();
        resource.managing_organization.push(item);
        resource
    }
...
impl crate::traits::care_team::CareTeamExistence for CareTeam {
    ...
    fn has_managing_organization(&self) -> bool {
        !self.managing_organization.is_empty()
    }
