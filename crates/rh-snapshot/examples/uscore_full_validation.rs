...
fn main() -> Result<()> {
    println!("🧪 Comprehensive US Core Validation Report\n");
    println!("Testing all US Core StructureDefinitions against official snapshots\n");
    ...
    for (name, version) in &[("hl7.fhir.r4.core", "4.0.1"), ("hl7.fhir.us.core", "6.1.0")] {
    ...
    println!("\n📋 Finding all US Core profiles...");
    let uscore_dir = packages_dir.join("hl7.fhir.us.core#6.1.0").join("package");

    let mut profiles = Vec::new();
    for entry in std::fs::read_dir(&uscore_dir)? {
        let entry = entry?;
        let path = entry.path();
    ...
    println!(
        "   Found {} US Core profiles with snapshots\n",
        profiles.len()
    );
    ...
    if perfect == results.len() && errors.is_empty() {
        println!(
            "   ✅ PERFECT! All {} US Core profiles match official snapshots exactly!",
            results.len()
        );
        println!("   rh-snapshot is fully compliant with US Core Implementation Guide");
    } else {
        println!(
            "   Overall match rate: {}%",
            (perfect * 100) / results.len()
