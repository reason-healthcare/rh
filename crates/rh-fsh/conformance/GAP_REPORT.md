...
61 fixture comparisons pass, all 93 library tests pass, and rh-fsh emits the
same 920 `(resourceType, id)` pairs as SUSHI across 379 real-project FSH files.
There are no missing or extra resources.

Content parity remains incomplete. Of the 920 shared resources, 167 (18.2%)
match exactly after normalization and 753 (81.8%) have at least one JSON
difference. Two consecutive full runs produced identical project counts and
category maps after deterministic resource sorting and insertion-ordered
ValueSet grouping were added.
...
|---|---:|---:|---:|---:|---:|---:|---:|
| CARIN Blue Button | 71 | 134 | 134 | 0 | 0 | 86 | 48 |
| mCODE | 57 | 350 | 350 | 0 | 0 | 307 | 43 |
| Da Vinci CRD | 69 | 85 | 85 | 0 | 0 | 70 | 15 |
| Da Vinci DTR | 39 | 75 | 75 | 0 | 0 | 54 | 21 |
| Da Vinci PAS | 20 | 158 | 158 | 0 | 0 | 133 | 25 |
| IPS | 123 | 118 | 118 | 0 | 0 | 103 | 15 |
| **Total** | **379** | **920** | **920** | **0** | **0** | **753** | **167** |
...
resources to 0 missing, 0 extra, and 753 mismatched resources.
...
|---|---:|---:|---|
| JSON shape | 434 | 57.6% | Recursive extension arrays, Bundle entries, Parameters parts, repeated BackboneElements, CodeableConcept/Coding distinctions, and primitive shadow alignment |
| StructureDefinition | 234 | 31.1% | Root constraints, context, differential merging/order, slices, bindings, fixed/pattern values, cardinalities, and type profiles |
| Other | 65 | 8.6% | Nested rule context, reference details, uncategorized project-specific fields, and metadata paths that need narrower classifiers |
| Metadata | 9 | 1.2% | Resource-specific defaults, names, descriptions, and derived metadata |
| IG generation | 6 | 0.8% | Page trees, grouping, globals, parameters, and definition resource metadata |
| Terminology | 5 | 0.7% | Concept properties and remaining CodeSystem/ValueSet canonical/compose differences |
| Resource identity | 0 | 0.0% | Closed for the pinned corpus |

### JSON shape: 434 resources

This is the largest remaining group and is concentrated in mCODE (250), IPS
(62), CARIN (42), and PAS (39). Repeated first differences include:
