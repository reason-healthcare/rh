...
Defines the behaviour of `rh package lock` — the command that resolves unversioned canonical references in FHIR source resources against installed dependency packages and records the resolved mappings in `fhir-lock.json`. Also specifies how `rh package build` applies canonical pinning from the lock file.
