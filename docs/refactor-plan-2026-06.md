...
| WS2 Conformance | 🔶 partial (2026-06-13) | **Done:** 2.1 HL7 FHIRPath suite vendored (937 cases) + harness `tests/hl7_conformance.rs` with summary JSON; baseline 564/935 pass (60.3%). Gate deviation: 120 baseline wrong answers made "zero wrong answers" impossible day-one — harness enforces a **shrink-only known-wrong-answers baseline** instead (no new wrong answers, stale entries must be removed). 2.2 CONFORMANCE.md + SPEC_COVERAGE.md created. 2.8 68 FSH parser unit tests added — found & fixed 3 real parser bugs: URLs truncated at `http` in bindings/`contains`/`from valueset`; integer-valued quantities (`85 'kg'`) dropped their unit; single-quoted UCUM units kept their quotes. 2.10 was a stale audit claim — `parse_vcl_expression` exists and all README symbols verified present; no changes needed. 2.4 clinical age operators complete: audit claim "0/8" was stale — `AgeIn<unit>[At]` already evaluated; added the missing `CalculateAgeIn<unit>[At]` (explicit birthDate) forms + 8 tests; SPEC_COVERAGE updated (Clinical 100%, total 169/175 = 97%). 2.5 done: Ratio literals parse/emit/eval (ELM Quantity + Ratio nodes now evaluate), ToRatio, 1-arg Combine, Message, Children, Descendants — 11 new tests; Error category 100%, total 170/175. ToString(Quantity) aligned to HL7 suite format (`125cm`). 2.3 wave 3: toDecimal, encode/decode, escape/unescape, numeric eq fix — 658 pass (70.4%). Wave 4: sort(), aggregate()/$total, type() reflection, comparable(), string empty-propagation ({}.*) — 694 pass (74.2%), eval errors 95→57. **Wave 5 (2026-06-13): partial-precision datetime/time literal parsers** — `parse_datetime_literal` and `parse_time_literal` rewritten to accept FHIRPath spec grammar (`@2015T`, `@2018-03-01T10:30`, `@T14`, `@T10:30:00.000`, fractional seconds with TZ): 694→724 pass (74.2%→77.4%), parse_error 61→1 (sole remaining is unrelated unary-plus `+1<+2`). **Wave 6 (2026-06-13): precision-aware temporal comparison + equality** — new `operations::temporal` module parses Date/DateTime/Time into structured `TemporalParts` (year/month/day/hour/min/sec/fraction_ns/tz_offset_min) and compares per spec: precision mismatch at a non-fractional component → empty for `<`/`>`/`=`; fractional zero treated as equal to no fractional; TZ-aware instant comparison when both sides have TZ and full time; cross-type Date-vs-Time → not-equal; `~`/`!~` precision-mismatch → false/true (not empty, per suite). 724→760 pass (77.4%→81.3%). 27 baseline entries removed (testEquality19/23/24, testGreatorOrEqual23-27, testLessThan23-27, testLiterals testDateNotEqual* + testLiteralDateTimeTZ*, testNEquality13/15/17/18, all wave-5 baselined comparison cases). 11 unit tests in the new module. **Wave 7 (2026-06-13): DateTime arithmetic preserves sub-second precision + TZ** — `add_precision_to_datetime` rewritten to split off the TZ suffix (`Z`/`+HH:MM`/`-HH:MM`) cleanly, parse base with `%.f` to capture fractional, and re-emit with `%.3f` if input had any fractional component. 760→766 pass (81.3%→81.9%). All 6 baselined testPlusDate3-8 cases flipped to pass; baseline now back to its pre-wave-5 size. **Wave 8 (2026-06-13): choice-type Quantity construction** — when `Observation.value` resolves to `valueQuantity` via the choice-type access path, build a typed `FhirPathValue::Quantity` from the `{value, unit, code}` object directly instead of returning a generic Object. New `construct_typed_choice_value` helper covers Quantity/Age/Distance/Duration/Count/Money plus all the FHIR primitive suffixes (Boolean, Integer, Decimal, String/Code/Uri/…, Date/DateTime/Instant/Time). Quantity gained `.value`/`.unit` member access. 766→770 pass (81.9%→82.4%). 4 testObservations cases flipped (testPolymorphismIsA1/IsA2/AsA/AsAFunction). Existing fhir_extension_test updated to expect Quantity instead of Object for `%resource.value`. **Wave 9 (2026-06-13): partial-precision conversion validators** — `is_valid_date_string` / `is_valid_datetime_string` / `is_valid_time_string` rewritten on top of `parse_temporal` so the spec's partial-precision shapes are accepted (`'2015'`, `'2015-02'`, `'2015-02-04T14'`, `'14'`, `'14:34'`). 770→777 pass (82.4%→83.1%). 7 testTypes conversion cases flipped (StringYear/Month/HourConvertsTo{Date,DateTime,Time}, StringMinuteConvertsToTime). Two existing unit tests updated for spec correctness: `'2023-12-25'.toDateTime()` no longer pads with `T00:00:00` (partial-precision Date is itself a valid DateTime), and Time literal grammar requires 2-digit components (single-digit `'0:0:0'` is rejected). **Wave 10 (2026-06-13): namespaced type names + FHIR resource type matching** — `is()`/`as()`/`ofType()` function-form now accepts qualified-name arguments (`System.Boolean`, `FHIR.Patient`, `FHIR.\`Patient\``) via a new `resolve_type_argument` helper that extracts the dotted chain from the parameter expression. `value_matches_type` gained explicit `fhir.*` arms for all FHIR primitive types and uses a `fhir.` prefix strip in the resource-type fallback so `Patient.is(FHIR.Patient)` matches. 777→794 pass (83.1%→84.9%), eval_error 59→41. One newly-runnable case (testType14: `Patient.active.is(System.Boolean).not()`) baselined — it requires the engine to track FHIR-vs-System primitive provenance, which all variants currently conflate (FhirPathValue::Boolean). **Wave 11 (2026-06-13): UCUM duration units in Date/DateTime arithmetic** — `parse_precision_unit` extended to recognise UCUM abbreviations (`a`/`mo`/`wk`/`d`/`h`/`min`/`s`/`ms`) alongside the calendar-duration names. Date-arithmetic call sites gate `'mo'` and `'a'` to an execution error since UCUM mean-month (30.44 d) and mean-year (365.25 d) are ambiguous over calendar boundaries; DateTime-arithmetic accepts them all. 794→801 pass (84.9%→85.7%). 7 testPlusDate cases flipped (15/18/19/20/21/22 + matching invalid-execution cases 14/16/17 still error correctly). **Wave 12 (2026-06-13): equivalence `~` semantics** — `values_equivalent` rewritten per FHIRPath §6.1.5: strings collapse runs of whitespace and compare case-insensitively (`'a' ~ 'A'` → true), decimals round to the smaller fractional precision (`1.2/1.8 ~ 0.67` → true), collections compare as multisets (`(1|2|3) ~ (3|2|1)` → true). 801→807 pass (85.7%→86.3%). 6 stale baseline entries removed (testEquivalent6/11/21/23, testNotEquivalent6/21). **Wave 13 (2026-06-13): toString polish + UCUM-quoting** — `Decimal.toString()` preserves the decimal point (`1.0` → `"1.0"`) so a Decimal can be distinguished from an Integer in string form; `Quantity.toString()` emits calendar-duration units unquoted (`1 week` → `"1 week"`) while UCUM codes stay quoted (`1 'kg'`). Three existing local unit tests updated for spec correctness. 807→810 pass (86.3%→86.6%). **Wave 14 (2026-06-13): toQuantity default unit + loose-string parser** — `toQuantity()` defaults the unit to the UCUM "no unit" code `'1'` for Integer / Number / Long / Boolean inputs per spec §6.3, and `parse_quantity_string` now accepts unquoted calendar-duration form (`'1 day'`) alongside the existing UCUM-quoted form. Two existing local unit tests updated. 810→816 pass (86.6%→87.3%). 6 stale baseline entries removed (testBooleanLiteralConvertsToQuantity, testDecimalLiteralToQuantity, testIntegerLiteralToQuantity, testStringQuantityLiteralConvertsToQuantity, testStringQuantityLiteralToQuantity, testStringDecimalLiteralToQuantity). **Wave 15 (2026-06-13): three-valued or/xor + string-function spec corrections** — `evaluate_or` now propagates the empty (unknown) operand through the FHIRPath three-valued logic table (§6.5): `or` resolves to `true` if either side is true (even with the other unknown) and to `empty` when one side is false and the other unknown; `xor` returns empty if either operand is unknown. `substring()` returns empty for negative or out-of-range `start`, propagates empty input, and accepts Long indices. `replaceMatches()` propagates empty regex/substitution arguments and treats an empty pattern as no-match (returns input unchanged) instead of splicing the substitution between every character. 816→830 pass (87.3%→88.8%), eval errors 34→31. 11 stale baseline entries removed (testBooleanLogicOr6/8/9, testBooleanLogicXOr3/6/7/8/9, testSubstring4/5, testReplaceMatches2). **Wave 16 (2026-06-13): UCUM duration names in unit converter + Quantity equivalence/division** — `UnitConverter` map extended with all FHIRPath calendar-duration names (`day/days/week/weeks/…/millisecond/milliseconds`) mapped to the same Time base unit so `7 days = 1 week` and `7 days = 1 'wk'` compare correctly. `values_equivalent` gained a Quantity arm that converts both sides to base units and compares at the precision of the less-precise value (significant figures), so `4 'g' ~ 4040 'mg'` → true. `divide_quantities` updated to return `Quantity{result, "1"}` (the UCUM dimensionless unit) instead of a bare Number, so `1.0 'm' / 1.0 'm' = 1 '1'` compares correctly. Three unit-test files updated to reflect the Quantity return type. 830→839 pass (88.8%→89.7%). 7 stale baseline entries removed (testQuantity4/5/6/11, testEquivalent22, testStringQuantityDayLiteralToQuantity, testStringQuantityWeekLiteralToQuantity). Total: 564→839 (60.3%→89.7%). resolve()/memberOf() deferred. 2.9 done: FSH fixture corpus expanded 47→61 files (14 new, covering slicing, choice-type, add-element, obeys, hierarchical codes, complex extensions, bundle instances, nested caret paths, filter operators, dollar-sign aliases, multi-profile, parameterized rulesets, ruleset reuse); CONFORMANCE.md written. All 61 fixtures parse cleanly. 2.6 done: CQL HL7 harness burn-down wave 1 (DateTime/Date/Time constructors, DateTimeComponentFrom, Before/After temporal operators, timezone-normalized datetime comparison, ToTime timezone fix) + wave 2 (Interval/List/temporal expected-value output parsers, Power/arithmetic expected-value expressions, sort desc fix, distinct-null, union multiset, Take/null, Split/null): **552→733 pass, 244→97 skip, 0 failures**. Engine bugs fixed: `sort desc` query clause, `distinct` null removal, `union` multiset semantics, `slice`/Take null, `split` null separator. 4 interval-except boundary-representation cases deferred in known-wrong baseline. **Remaining:** 2.3 sub-waves: defineVariable, typed-primitive variant (testInheritance 12 + testType14), strict-mode polymorphism (testPolymorphismB), partial-precision boundary functions (lowBoundary/highBoundary/precision on partial DateTimes), unary `+1` literal in comparisons, iif() semantics edge cases (testIif6/9/10/11 — short-circuit and multi-item criteria), integer-to-boolean coercion in `.not()`, UCUM cross-unit multiplication/division (testQuantity9/10), sort descending by expression (testSort5/8/10); 2.6 CQL skip burn-down (97 → <50 target); 2.7 (ELM fidelity diff). WS2 FHIRPath baseline: 848/935 (90.7%). CQL: 733/~1000+ estimated (many in eval_err/compile_err). **Wave 17 (iif criterion validation) + Wave 18:** iif() criterion must be Boolean or empty; TypeError/EvaluationError on non-boolean inputs. testCollectionBoolean1 and testIif6 now pass. 848→850 pass (90.7%→90.9%). **Wave 19 (2026-06-13): math edge cases + string empty propagation + union dedup + exists/all criterion:** div/mod by zero → Empty; sqrt(neg) → Empty; power(NaN) → Empty; abs(Quantity); exp/length/trim on Empty → Empty; replace() with empty params → Empty; decimal subtraction/mod rounding to input precision (1.8-1.2=0.6 ✓); .not() on non-boolean → Boolean(false); implies three-valued (true implies {} → Empty); union | deduplicates per spec; exists(criterion)/all(criterion) with $this per-element binding; try_temporal also parses String for inequality comparison. 850→873 pass (90.9%→93.4%). 10 baseline entries removed. **Wave 20 (2026-06-13): lazy iif + sort by expression + pound unit aliases:** iif() now evaluates criterion first and only evaluates the selected branch (lazy); sort(key...) supports expression keys with `-expr` meaning sort descending; sort compare treats Empty as "last" (greater than all); '[lb_av]'/'lbs' added as mass unit aliases. testCollectionBoolean5/6 (iif short-circuit), testAggregate3/4, testIif9/11 (receiver as $this), testSort5/8/10 (descending sort), testEquality28, testLessThan/GreaterThan22 now pass. 873→885 pass (93.4%→94.7%). testIif9/11, testEquality28 removed from baseline. **Wave 21 (2026-06-13): repeat() spec fix:** repeat() now returns only projected items (excludes initial input), matching FHIRPath §6.6. testRepeat1/5 now pass. 885→887 pass (94.7%→94.9%). **Remaining:** testInheritance (12 — FHIR primitive provenance), testType9/10/12/14 (same), testVariables4 (backtick var), testExtension1 (extension on primitive), testPrecedence3/4 (is precedence), testCombine1 ($this.descendants()), testWhere2/4 / testIndexer2 (collection=scalar), testMatches (single-line mode), testIif10 (multi-item iif receiver), testQuantity9/10 (unit algebra), testPolymorphism*, Boundary/Precision (f64 limits). CQL skip burn-down (97→<50). |
| WS3 CLI | ⬜ not started | |
...
**Status 2026-06-13:** 2.1 ✅ · 2.2 ✅ · 2.4 ✅ · 2.5 ✅ · 2.6 ✅ (waves 1–2 done; further skip burn-down remaining) · 2.8 ✅ · 2.9 ✅ · 2.10 ✅ (no-op, stale audit claim) · 2.3 waves 1–17 done; sub-waves remaining (defineVariable, typed-primitive provenance, UCUM cross-unit multiplication/division, partial-precision boundary funcs, iif edge cases) · 2.7 not started. WS5 regression net is in place at 848/935 (90.7%). See Progress Tracking for details.
