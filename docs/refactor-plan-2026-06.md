...
| WS4 Codegen | 🔶 in progress (2026-06-15) | **4.1✅** regen harness + CI drift. **4.2✅** R5 patches. **4.3✅** golden tests. **4.4✅** god-file splits: trait_impl_generator→4 files (732/225/388/262), file_generator→3 files (639/436/94)+moved crate_gen methods, all prod code <800 LOC. **4.5✅** borrow-checker clones. **4.6✅** unwraps. **4.7✅** rayon parallelization: Arc<Mutex<>> for type_cache/enum_cache/value_set_manager, `generate_structs_parallel` + `write_all_generated_files` + `write_all_trait_files`, `pre_generate_base_definitions` optimization, criterion bench suite. **4.8✅** (needs R4/R5 regen): Vec<T> with serde(default). **4.9✅** Box doc. **4.10✅** metadata split: monolith metadata.rs → category-based metadata/ directory (resources.rs, datatypes.rs, primitives.rs, mod.rs) for incremental compile. **4.12✅** parse tests. **Pending:** 4.11 R5 extensions. |
