...
| WS4 Codegen | 🔶 in progress (2026-06-15) | **4.1 done:** regen harness + CI drift job + golden/parse tests. **4.2 done:** all R5 patches already in codegen (SubscriptionStatus collision, language binding, integer64, MetadataResource differential, cardinality always-emit). **4.3 done:** golden-file tests for resource/datatype/primitive/profile/trait. **4.5 done:** removed borrow-checker clones in generate_enums_mod_file (changed &self → &mut self). **4.6 done:** replaced all 26 production unwraps — path parsing uses CodegenError::MissingField, cached lookups have graceful fallbacks, char ops use safe alternatives, hardcoded strings use descriptive .expect("codegen bug: …"). **4.12 done:** property-based parse tests. |
