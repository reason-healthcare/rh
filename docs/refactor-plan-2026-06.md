...
| WS4 Codegen | 🔶 in progress (2026-06-15) | **4.1✅** regen harness + CI drift job + golden/parse tests. **4.2✅** R5 patches in codegen. **4.3✅** golden-file tests. **4.5✅** removed borrow-checker clones. **4.6✅** replaced all production unwraps. **4.8✅** (needs R4/R5 regen + version bump): `Option<Vec<T>>` → `Vec<T>` with serde(default). **4.9✅** Box usage doc. **4.12✅** parse tests. **Pending:** 4.4 god-file split, 4.7 rayon, 4.10 metadata split, 4.11 R5 extensions. |
