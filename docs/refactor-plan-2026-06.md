...
| WS6 WASM/NPM | 🔶 in progress (2026-06-15) | 6.1 done: root `just wasm-build <crate> <target>` and `just wasm-check` added; CI now compile-checks foundation, fhirpath, vcl, and cql for `wasm32-unknown-unknown`. 6.2 done: pnpm workspace with `@reason-healthcare/fhirpath` and `@reason-healthcare/vcl` TS wrappers, node/web/bundler exports, Vitest suites, README files, and npm pack dry-runs. 6.3 deferred by maintainer decision. 6.4 TODO. 6.5 done: `rh-cql` WASM feature/export module plus `@reason-healthcare/cql` package. |
