...
|---|---|---|---|
| 0.1 | Fix C1: change R5 dep to `rh-foundation = { path = "../rh-foundation", version = "0.2.0" }`; adopt workspace inheritance for version/edition/authors/license/rust-version; align crate metadata (description/keywords/categories) with R4 crate. | `crates/rh-hl7_fhir_r5_core/Cargo.toml` | `cargo metadata` resolves on a clean clone; `cargo build -p rh-hl7-fhir-r5-core` passes |
...
| 4.7 | ✅ Parallelize generation: rayon `par_iter` over StructureDefinitions for token generation + file writing. Cache parsed base definitions (Element, BackboneElement, DomainResource) as shared singletons via `pre_generate_base_definitions`. `CodeGenerator` uses `Arc<Mutex<>>` for type_cache/enum_cache/value_set_manager. CLI uses `generate_structs_parallel` + `write_all_generated_files` + `write_all_trait_files`. Criterion bench suite in `benches/generate.rs`. | `crates/rh-codegen/src/{generator,file_generator}`, `apps/rh-cli/src/codegen.rs` | criterion bench `benches/generate.rs` added; R4 full generation wall-time recorded before/after in PR description; output byte-identical to serial run |
...
| 4.10 | ✅ Split `metadata.rs` monoliths by category (R4 25.9k / R5 30.5k LOC) → `metadata/` directory with `resources.rs`, `datatypes.rs`, `primitives.rs`, `profiles.rs`, `other.rs`, `mod.rs`. Improves incremental compile. | `crates/rh-codegen/src/generators/metadata_generator.rs`, CLI codegen | Build-time delta recorded; no API change |
| 4.11 | ✅ R4 extensions story: R4 has 388 generated extension files, R5 only 3 — regenerate R5 extensions for parity; add typed extension accessor helpers (e.g., `resource.extension_by_url(url)` in foundation trait) to fix the "weak extension story". | `crates/rh-codegen/src/generators/accessor_trait_generator.rs`, `justfile` | R5 regen includes extensions; `extension_by_url()` on DomainResourceAccessors+ElementAccessors |
...
Depends on WS1 (1.6 foundation wasm hygiene). Completed scope: rh-fhirpath, rh-vcl, and rh-cql have wasm-bindgen exports, `wasm` features, wasm-pack metadata (`-O4`), shared root just recipes, npm workspace packages, Vitest coverage, and a shared Vite playground. NPM release automation (6.3) and rh-fsh WASM (6.6) are deferred by maintainer decision.
...
| 6.3 | ⏸ Deferred: NPM release automation: extend `release.yml` with a job building wasm + publishing packages on `v*` tags (npm provenance, `NPM_TOKEN` secret); document in RELEASE.md (and fix RELEASE.md pointing at WASM_BUILD.md docs — verify paths). | `.github/workflows/release.yml`, `RELEASE.md` | Dry-run publish (`npm publish --dry-run`) wired into CI |
...
| 7.4 | ✅ RELEASE.md: npm publish flow (6.3), regenerated-crates release policy (when codegen changes require R4/R5 crate version bumps), verify WASM doc paths. | `RELEASE.md` | Dry-run release checklist executed once |
