...
| WS2 Conformance | 🔶 partial (2026-06-14) | **Done:** 2.1 HL7 FHIRPath suite vendored (937 cases) + harness `tests/hl7_conformance.rs` with summary JSON; baseline 564/935 pass (60.3%) and shrink-only known-wrong-answer gating. 2.2 `CONFORMANCE.md` + `SPEC_COVERAGE.md` created and kept current. 2.4 clinical age operators complete. 2.5 Ratio literals, ToRatio, 1-arg Combine, Message, Children, Descendants complete. 2.6 CQL HL7 burn-down waves 1–2 landed: 552→733 pass, 244→97 skip, 0 failures. 2.7 fixed-corpus ELM fidelity diff landed in `pipeline_comparison_tests.rs`, with Java-vs-Rust metadata reporting documented in `crates/rh-cql/CONFORMANCE.md`. 2.8 68 FSH parser unit tests added and 3 parser bugs fixed. 2.9 FSH fixture corpus expanded to 61 files. 2.10 stale audit claim closed with no code changes. **FHIRPath waves 3–32:** 564→923 pass (60.3%→98.7%), wrong answers 120→10, parse errors 79→0, eval errors 171→1. Wave 32 (2026-06-14): unary `+` parser; `$index` in `select()`/`where()`; `is` returns empty for empty input (spec); `TypedObject` variant + `FhirPrimitive` wrapper enabling `ofType(HumanName)`, `value is Age/Quantity`, primitive extension access (`_birthDate`), `%ext-` variable expansion, `conformsTo()` stub, `as()` errors on multi-element collections, codesystem fixture nested concepts. **Remaining FHIRPath work:** decimal trailing-zero precision (`LowBoundary*`, `HighBoundary*`, `PrecisionDecimal`) needs `rust_decimal`; strict polymorphic access (`testPolymorphismB`, `testPolymorphicsB`, `testDollarOrderNotAllowed`) needs strict mode; `testExtension2` edge case; `testDateTimeGreaterThanDate2` timezone-flaky. **Pending non-FHIRPath WS2 work:** CQL skip burn-down (97 → <50 target). |
